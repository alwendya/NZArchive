...
namespace nsEXTRACT
{
        ...
        PCWSTR pszFriendlyName, PCWSTR pszThreadModel)
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = SetHKCRRegistryKeyAndValue(szSubkey, NULL, pszFriendlyName, HKEY_CURRENT_USER);
            if (SUCCEEDED(hr))
            {
                ...
                if (SUCCEEDED(hr))
                {
                    hr = SetHKCRRegistryKeyAndValue(szSubkey, NULL, pszModule, HKEY_CURRENT_USER);
                    if (SUCCEEDED(hr))
                    {
                        hr = SetHKCRRegistryKeyAndValue(szSubkey, 
                            L"ThreadingModel", pszThreadModel, HKEY_CURRENT_USER);
                    }
                }
            }
        }
    ...
    HRESULT UnregisterInprocServer(const CLSID& clsid)
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = HRESULT_FROM_WIN32(SHDeleteKeyW(HKEY_CURRENT_USER, szSubkey));
        }
    ...
    HRESULT RegisterShellExtOverlayIconHandler(PCWSTR pszName, const CLSID& clsid)
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = SetHKCRRegistryKeyAndValue(fullPath.c_str(), NULL, szCLSID, HKEY_CURRENT_USER);
        }
    ...
    HRESULT RegisterShellExtContextMenuHandler(PCWSTR pszFileType, const CLSID& clsid, PCWSTR pszFriendlyName)
    {
        ...
        fullPath = L"SOFTWARE\\CLASSES\\" + std::wstring(pszFileType) + kContextMenuRegistryPath + pszFriendlyName;
        if (SUCCEEDED(hr))
        {
            hr = SetHKCRRegistryKeyAndValue(fullPath.c_str(), NULL, szCLSID, HKEY_CURRENT_USER);
        }
        ...
        PCWSTR pszFriendlyName)
    {
        ...
        std::wstring fullPath = L"SOFTWARE\\CLASSES\\" + std::wstring(pszFileType) + kContextMenuRegistryPath + pszFriendlyName;

        if (SUCCEEDED(hr))
        {
            hr = HRESULT_FROM_WIN32(SHDeleteKey(HKEY_CURRENT_USER, fullPath.c_str()));
        }
