...
const std::wstring kExtensionsName(L"NZArchiveExtract");
...
STDAPI DllGetClassObject(REFCLSID rclsid, REFIID riid, void** ppv)
{
	...
	if (iter != globalData.end())
	{
		...
		nsEXTRACT::ClassFactory* pClassFactory = new nsEXTRACT::ClassFactory(iter->m_Func);
...
STDAPI DllRegisterServer(void)
{
	...
	{
		hr = nsEXTRACT::RegisterInprocServer(szModule, iter->m_CLSID, kExtensionsName.c_str(), L"Apartment");
		...
		if (SUCCEEDED(hr))
		{
			if (iter == factoryData.end() - 1)
			{
				...
				hr = nsEXTRACT::RegisterShellExtContextMenuHandler(L"SOFTWARE\\CLASSES\\.NZA", /* * to AllFileSystemObjects */
					iter->m_CLSID,
			...
			else
			{
				...
				hr = nsEXTRACT::RegisterShellExtOverlayIconHandler(ws.str().c_str(), iter->m_CLSID);
			}
		}
		counter++;
	}
...
STDAPI DllUnregisterServer(void)
{
	...
	for (auto iter = factoryData.begin(); iter != factoryData.end(); ++iter)
	{
		hr = nsEXTRACT::UnregisterInprocServer(iter->m_CLSID);
		...
		if (SUCCEEDED(hr))
		{
			if (iter == factoryData.end() - 1)
			{
				...
				hr = nsEXTRACT::UnregisterShellExtContextMenuHandler(L"SOFTWARE\\CLASSES\\.NZA",
					iter->m_CLSID,
					iter->m_friendlyName.c_str());
			}
			else
			{
				...
				ws << counter;
				ws << iter->m_friendlyName.c_str();
				hr = nsEXTRACT::UnregisterShellExtOverlayIconHandler(ws.str().c_str(), iter->m_CLSID);
			}
		}
