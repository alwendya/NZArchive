...
const std::wstring kExtensionsName(L"NZArchiveADD");
...
STDAPI DllGetClassObject(REFCLSID rclsid, REFIID riid, void** ppv)
{
	...
	if (iter != globalData.end())
	{
		...
		nsADDTO::ClassFactory* pClassFactory = new nsADDTO::ClassFactory(iter->m_Func);
...
STDAPI DllRegisterServer(void)
{
	...
	{
		hr = nsADDTO::RegisterInprocServer(szModule, iter->m_CLSID, kExtensionsName.c_str(), L"Apartment");
		...
		if (SUCCEEDED(hr))
		{
			if (iter == factoryData.end() - 1)
			{
				...
				hr = nsADDTO::RegisterShellExtContextMenuHandler(L"SOFTWARE\\CLASSES\\AllFileSystemObjects", /* * to AllFileSystemObjects */
					iter->m_CLSID,
			...
			else
			{
				...
				hr = nsADDTO::RegisterShellExtOverlayIconHandler(ws.str().c_str(), iter->m_CLSID);
			}
		}
		counter++;
	}
...
STDAPI DllUnregisterServer(void)
{
	...
	for (auto iter = factoryData.begin(); iter != factoryData.end(); ++iter)
	{
		hr = nsADDTO::UnregisterInprocServer(iter->m_CLSID);
		...
		if (SUCCEEDED(hr))
		{
			if (iter == factoryData.end() - 1)
			{
				...
				hr = nsADDTO::UnregisterShellExtContextMenuHandler(L"SOFTWARE\\CLASSES\\AllFileSystemObjects",
					iter->m_CLSID,
					iter->m_friendlyName.c_str());
			}
			else
			{
				...
				ws << counter;
				ws << iter->m_friendlyName.c_str();
				hr = nsADDTO::UnregisterShellExtOverlayIconHandler(ws.str().c_str(), iter->m_CLSID);
			}
		}
